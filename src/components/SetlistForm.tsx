...
export function SetlistForm({ setlist, onSuccess }: SetlistFormProps) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      date: setlist?.date?.seconds
        ? new Date(setlist.date.seconds * 1000).toISOString()
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    if (songIds.length === 0 && chantIds.length === 0) {
